...
for i in range(8):
    ...
    for new_beta in beta:
        negated_new_bate = '~'+new_beta;
        if(not a.pl_resolution(knowledgebase,negated_new_bate) and not a.pl_resolution(knowledgebase,new_beta)):
            print(new_beta+":Unprovable",end=", ");
            continue;

        if a.pl_resolution(knowledgebase,new_beta):
            print(new_beta+':true',end=", ")
        else:
            print(new_beta+':false',end=", ")
    end = time.clock();
    print("")
